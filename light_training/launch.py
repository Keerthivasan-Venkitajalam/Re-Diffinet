...
def launch_dist(
    env_type="DDP",
    num_nodes=1,
    gpus_per_node=1,  # Set this to 1 for single-GPU or CPU setups
    master_addr='localhost',
    master_port=17500,
    training_script='train.py',
):
    if num_nodes != 1:
        print("Multi-node multi-GPU training is not supported yet.")
        os._exit(0)
    if env_type == "DDP":
        cmd_launch = [
            "torchrun",
            "--nproc_per_node",
            str(gpus_per_node),  # Ensure this matches the number of GPUs
            "--nnodes",
            str(num_nodes),
            "--node_rank",
            str(0),
        ...
        if os.name == 'nt':  # Windows
            p = subprocess.Popen(run_cmd, shell=True, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
        else:
